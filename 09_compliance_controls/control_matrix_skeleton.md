...
| CTRL-025 **(HOSTED at ONBOARD-1b, ratified OQ-ONB-9A 2026-08-09: the four-eyes maker-checker lifecycle — ENT-075 `entitlement_request`, `role.approve`, threshold ≥1 other admin, admin-deactivation INCLUDED — moves this row on the P9 bar when 1b lands.)** | Entitlement changes maker-checked + audited | Preventive | Automated | §13 | BR-7, BR-11 | R-07 | Entitlement-change test | `ENTITLEMENT.*` events | Planned |
...
| CTRL-037 | Tenant orphan-proofing: a tenant always has ≥1 valid active admin | Preventive | Automated | §6 | BR-11 | R-07 | The OQ-ONB-4A invariant over ALL four paths (revoke, end-date, deactivate, concurrent — per-tenant advisory lock) | **Planned — HOSTED at ONBOARD-1b** (the enforcement paths land with the tenant-local admin verbs; in 1a the invariant's only exercise is birth: a tenant cannot be created without its first admin, `test_a_missing_admin_template_FAILS_CLOSED` + mutant N-A6) | **Planned (host: ONBOARD-1b, sequenced under the ratified ONBOARD-1 gate).** Named at the mint rather than at a close review — P19's deferral discipline applied to a control row. |
