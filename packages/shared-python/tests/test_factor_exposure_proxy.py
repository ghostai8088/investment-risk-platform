...
def _book(db: Session, tenant: str, holdings: list[tuple[str, str, str, str]]) -> tuple[str, str]:
    """Seed a portfolio with (code, qty, mark, mark_ccy) holdings + a COMPLETED exposure run.
    Returns (exposure_run_id, {code: instrument_id})... returns (run_id, portfolio_id) and stores
    instrument ids on the function attr for simplicity."""
    pf = create_portfolio(
        db,
        tenant_id=tenant,
    ...
    _book.insts = insts  # type: ignore[attr-defined]
    return exposure.run.run_id, pf
...
def _seed_mixed(db: Session, tenant: str):  # noqa: ANN202
    ...
    pe = _book.insts["I-PE"]  # type: ignore[attr-defined]
    _proxy(db, tenant, pe, fid_usd, "0.6")
    _proxy(db, tenant, pe, fid_eur, "0.3")
    return exp_run, fid_usd, fid_eur, pe


def _run_proxy(db: Session, tenant: str, exp_run: str, fids: list[str], **kw):  # noqa: ANN202
...
def test_proxy_projection_golden_mixed_book(session: Session) -> None:
    t = str(uuid.uuid4())
    exp_run, fid_usd, fid_eur, pe = _seed_mixed(session, t)
    ...
    usd_inst = _book.insts["I-USD"]  # type: ignore[attr-defined]
...
def test_end_to_end_var_equals_public_equivalent(session: Session) -> None:
    ...
    t = str(uuid.uuid4())
    exp_run, fid_usd, fid_eur, _pe = _seed_mixed(session, t)
    ...
    pub_run, _pf = _book(
        session, t, [("I-A", "200", "300.00", "USD"), ("I-B", "50", "300.00", "EUR")]
    )
...
def test_no_proxy_rows_degrades_to_indicator(session: Session) -> None:
    ...
    assert rows["FX_EUR"].exposure_amount == Decimal("40000.000000")
