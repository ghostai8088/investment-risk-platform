"""PostgreSQL-only proofs for P3-5 ``var_result`` (ENT-027 realized, IA — parametric VaR v1),
run as the constrained NOSUPERUSER/NOBYPASSRLS ``irp_app`` role (the CI/pipeline posture):
...
- the symmetric policy shape + the closed 5-table hybrid set unchanged (probed by the REAL
  SYSTEM_TENANT_ID; set EQUALITY);
...
factor-exposure run + covariance run -> VaR run) executes through the binders under
``set_tenant_context`` — the deepest governed composition exercised on PG so far.
"""
...
def app_url() -> str:
    ...
    with superuser.begin() as conn:
        ...
        for table in (*_P3_5, *_RUN, *_SNAP, *_DEPS, *_RAILS):
...
def _seed_and_run(factory, tenant: str) -> str:  # noqa: ANN001
    """The FULL chain: holdings -> exposure run -> factor-exposure + covariance runs -> a
    COMPLETED VaR run (its run_id)."""
    session = factory()
    try:
...
def test_tenant_isolation(app_url: str) -> None:
    ...
    try:
        ...
        rows = session.execute(text("SELECT calculation_run_id FROM var_result")).fetchall()
...
def test_no_context_read_returns_zero_rows(app_url: str) -> None:
    ...
    session = factory()
    try:
        count = session.execute(text("SELECT count(*) FROM var_result")).scalar_one()
...
def test_append_only_trigger_blocks_update_and_delete(app_url: str) -> None:
    ...
    try:
        ...
        with pytest.raises(ProgrammingError) as upd:
            session.execute(text("UPDATE var_result SET var_value = 0"))
        assert _is_append_only_violation(upd.value)
        session.rollback()
        set_tenant_context(session, tenant)
        with pytest.raises(ProgrammingError) as dele:
            session.execute(text("DELETE FROM var_result"))
    ...
    try:
        ...
        row = session.execute(
            text(
                ...
                "exposure_run_id, covariance_run_id FROM var_result LIMIT 1"
            )
        ).one()
        with pytest.raises(ProgrammingError) as forged:
            session.execute(
                text(
                    "INSERT INTO var_result (id, tenant_id, system_from, calculation_run_id, "
                    "input_snapshot_id, model_version_id, exposure_run_id, covariance_run_id, "
                    "metric_type, base_currency, confidence_level, horizon_days, z_score, "
                    "sigma, var_value, n_factors, n_observations, window_start, window_end) "
                    "VALUES (:id, :victim, now(), :run, :snap, :mv, :er, :cr, "
                    "'VAR_PARAMETRIC', 'USD', 0.95, 1, 1.644853626951, 1, 1, 2, 4, :ws, :we)"
                ),
                {
                    "id": str(uuid.uuid4()),
    ...
    try:
        with engine.connect() as conn:
            qual, check = conn.execute(
                text("SELECT qual, with_check FROM pg_policies WHERE tablename = 'var_result'")
            ).one()
            ...
            # Probe by the REAL system-tenant id; EQUALITY (the 2026-07 P3-4 review lesson).
...
def test_cross_tenant_snapshot_consume_fails_closed(app_url: str) -> None:
    ...
    try:
        ...
        snap = session.execute(
            text(
                ...
                "WHERE run_type = 'VAR' AND tenant_id = :t LIMIT 1"
            ),
            {"t": t1},
        ).scalar_one()
    finally:
    ...
    try:
        ...
        mv = register_var_model(
            session, tenant_id=t2, actor_id="a", code_version="risk-v1", confidence_level="0.95"
        )
        session.flush()
