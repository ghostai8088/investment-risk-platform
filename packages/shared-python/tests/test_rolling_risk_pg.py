"""RM-1 PostgreSQL enforcement tier for ENT-064 ``rolling_risk_result``.

**This file exists because the 4-finder review found ENT-064 was the ONLY governed result family
without one.** Every sibling has a PG suite; RM-1 shipped its SQLite service tier and its demo stage
and nothing that exercises the database's own guarantees. Those guarantees are ORM-invisible by
construction — the CHECK and the RLS policy are created imperatively in the migration, and the
append-only trigger is a PL/pgSQL function — so the entire SQLite battery is blind to all of them.

Four controls, each with its reject side EXECUTED (the standing rule: a guard never demonstrated to
fire is not a guard):

1. the suppression CHECK refuses both incoherent states, and accepts both coherent ones;
...
4. tenant RLS hides another tenant's rows from a NON-BYPASSRLS role, with the explicit predicate
   and the policy checked SEPARATELY so neither can mask the other's absence.
"""
...
_CHECK = "ck_rolling_risk_result_suppression_coherent"
_GRAIN = "uq_rolling_risk_result_run_grain"
...
def app_url() -> str:
    ...
    with superuser.begin() as conn:
        ...
        # The referent tables are granted because the seeder INSERTs into them as irp_app. Declared
        # here rather than inherited from another suite's fixture: the SCH-2 review found a file
        # that was green in CI only by GRANT LEAKAGE from ~30 earlier suites in the same job.
        for table in (
            "rolling_risk_result",
            "calculation_run",
            "dataset_snapshot",
...
def _seed_referents(conn, tenant: str) -> dict[str, str]:  # noqa: ANN001
    """Real FK referents. Random UUIDs would make every INSERT fail on the FK instead of the
    constraint under test — the matrix would then 'pass' for entirely the wrong reason."""
    ids = {
        k: str(uuid.uuid4())
        for k in ("portfolio", "run", "return_run", "snapshot", "model", "model_version")
    }
    conn.execute(
        text(
    ...
    for key, run_type in (("run", "ROLLING_RISK"), ("return_run", "PORTFOLIO_RETURN")):
        conn.execute(
            text(
    ...
    conn.execute(
        text(
            ...
            " VALUES (:id, :t, now(), now(), now(), '', 'ROLLING_RISK_INPUT', now(), now(),"
            " :vd, 'v1:test', 0, :h)"
        ),
...
def _insert_row(conn, tenant: str, ids: dict[str, str], **over) -> None:  # noqa: ANN001, ANN003
    ...
    conn.execute(
        text(
            "INSERT INTO rolling_risk_result (id, tenant_id, system_from, calculation_run_id,"
            " input_snapshot_id, model_version_id, portfolio_id, portfolio_return_run_id,"
            " metric_type, window_months, period_start, period_end, metric_value, suppressed,"
            " suppression_reason, annualization_basis, sampling_frequency, n_observations)"
            " VALUES (:id, :tenant, now(), :run, :snapshot, :model_version, :portfolio,"
            " :return_run, :metric_type, :window_months, :period_start, :period_end,"
            " :metric_value, :suppressed, :suppression_reason, :annualization_basis,"
...
def test_the_suppression_check_refuses_every_incoherent_state(app_url: str) -> None:  # noqa: F811
    ...
    with engine.connect() as conn:
        ...
        incoherent = (
            ...
            (
                "emitted with a reason",
                {"suppressed": False, "suppression_reason": "why"},
            ),
            (
                "emitted with no value",
                {"suppressed": False, "metric_value": None},
            ),
        )
...
def test_the_check_matrix_actually_observes_the_constraint() -> None:
    ...
    with engine.connect() as conn:
        trans = conn.begin()
        conn.execute(text(f"ALTER TABLE rolling_risk_result DROP CONSTRAINT {_CHECK}"))
        tenant = str(uuid.uuid4())
        _arm(conn, tenant)
        ids = _seed_referents(conn, tenant)
...
def test_the_four_column_grain_refuses_a_duplicate(app_url: str) -> None:  # noqa: F811
    """The collision the grain exists to prevent — asserted in prose in the service suite, executed
    only here. Also proves the window is genuinely part of the key: the SAME
    (run, metric, period_start) at a DIFFERENT window is accepted, which is precisely why three
...
def test_the_append_only_trigger_refuses_update_and_delete(app_url: str) -> None:  # noqa: F811
    ...
    with engine.connect() as conn:
        ...
        for statement in (
            "UPDATE rolling_risk_result SET metric_value = 0.99 WHERE tenant_id = :t",
            "DELETE FROM rolling_risk_result WHERE tenant_id = :t",
        ):
            sp = conn.begin_nested()
...
def test_rls_hides_another_tenants_rows_from_the_app_role(app_url: str) -> None:  # noqa: F811
    ...
    with engine.connect() as conn:
        ...
        _arm(conn, a)
        mine = conn.execute(text("SELECT count(*) FROM rolling_risk_result")).scalar_one()
        ...
        foreign = conn.execute(
            text("SELECT count(*) FROM rolling_risk_result WHERE tenant_id = :t"), {"t": b}
        ).scalar_one()
        assert foreign == 0

        _arm(conn, b)
        assert conn.execute(text("SELECT count(*) FROM rolling_risk_result")).scalar_one() == 1
        trans.rollback()
    engine.dispose()
...
def test_the_isolation_test_would_notice_if_rls_were_off() -> None:
    ...
    with engine.connect() as conn:
        ...
        visible = conn.execute(
            text("SELECT count(*) FROM rolling_risk_result WHERE tenant_id = ANY(:ts)"),
            {"ts": tenants},
        ).scalar_one()
