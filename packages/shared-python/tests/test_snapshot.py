...
def test_nothing_imports_snapshot() -> None:
    ...
    components) + ``pacing`` (CC-2 — the pacing binder resolves the PACING_INPUT snapshot + reads
    ...
    the apps layer; nothing imports it). ``liquidity`` joined at LQ-1 on identical grounds. ``concentration`` joined at CON-1 (OQ-CON-1-19: the NEW
    ...
    for path in root.rglob("*.py"):
        if (
            ...
            # bind). Admitted by name rather than by loosening the rule: the fence's value is that
            # each member had to be argued for, and a wildcard would end that.
            or "liquidity" in path.parts
