...
def test_no_migration_and_no_entity() -> None:
    ...
    # 0038_var_residual_variance; the synthetic slice still adds no migration, so the next slot
    # (0039+) must remain empty.
    assert not list(versions.glob("0039*")), "no 0039 migration may be added by the synthetic slice"
