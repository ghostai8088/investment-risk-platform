...
def test_no_migration_and_no_entity() -> None:
    ...
    # P2-1..P2-5 legitimately own 0016/0017/0018/0019/0020; the synthetic slice still adds no
    # migration, so the next slot (0021+) must remain empty here (no surprise migration).
    assert not list(versions.glob("0021*")), "no 0021 migration may be added by the synthetic slice"
