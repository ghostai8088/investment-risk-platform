...
def test_no_migration_and_no_entity() -> None:
    ...
    # ES-HS-1 owns 0041_es_historical;
    # the synthetic slice still adds no migration, so the next slot (0042+) must remain empty.
    assert not list(versions.glob("0042*")), "no 0042 migration may be added by the synthetic slice"
