...
def test_no_migration_and_no_entity() -> None:
    ...
    # ES-HS-1 owns 0041_es_historical; DS-2 owns 0042_desmoothing_estimated_alpha; BT-3 owns
    ...
    # 0048_var_private_variance; SCH-1 owns 0049_scheduling; LIM-1 owns 0050_limit_breach; the
    # synthetic slice still adds no migration, so the next slot (0051+) must remain empty.
    assert not list(versions.glob("0051*")), "no 0051 migration may be added by the synthetic slice"
