...
def test_no_migration_and_no_entity() -> None:
    ...
    # PA-1 owns 0036_desmoothed_return; PA-3 owns 0037_proxy_weight_estimate; the synthetic slice
    # still adds no migration, so the next slot (0038+) must remain empty.
    assert not list(versions.glob("0038*")), "no 0038 migration may be added by the synthetic slice"
