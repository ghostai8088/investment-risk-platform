...
def test_no_migration_and_no_entity() -> None:
    ...
    # MG-2 owns 0051_breach_action; the synthetic slice still adds no migration, so the next
    # free slot (0052+) must remain empty.
    assert not list(versions.glob("0052*")), "no 0052 migration may be added by the synthetic slice"
