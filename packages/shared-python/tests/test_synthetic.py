...
def test_no_migration_and_no_entity() -> None:
    ...
    # PA-1 owns 0036_desmoothed_return; the synthetic slice still adds no migration, so the next
    # slot (0037+)
    # must remain empty.
    assert not list(versions.glob("0037*")), "no 0037 migration may be added by the synthetic slice"
