...
def list_var_backtests_by_entity(
    session: Session,
    *,
    acting_tenant: str,
    ...
    """API-1 entity/time read (Class A): ``var_backtest_result`` rows across COMPLETED
    **VAR_BACKTEST** runs for a portfolio (the ``run_type`` filter keeps es-backtest out of the
    shared table). Silent-empty on a foreign id; ``as_of=None`` = now."""
...
def latest_var_backtest(
    ...
    *,
    ...
    """API-1 latest-resolver (Class A): the newest COMPLETED VAR_BACKTEST run's rows for the
