"""Position governed-write core (P1C-3) — co-transactional, fail-closed audit + lineage.

A self-contained mirror of the established governed-write shape (``transaction.service`` /
``reference.service``); the rail layer imports only ``lineage`` / ``audit`` / ``db`` (the domain
resolvers ``resolve_portfolio``/``resolve_instrument`` are imported by the binder, not here):

    add(version) -> flush -> record_lineage(MANUAL source, ORIGIN) -> record_event(POSITION.*)
...
- ``record_position_create`` roots one ORIGIN lineage edge + emits ``POSITION.CREATE`` (a captured
new
  version — initial create OR the new open row of a supersede).
- ``record_position_update`` emits ``POSITION.UPDATE`` for a prior-head close-out — **no** new edge
  (the version keeps its origin edge); carries the changed boundary column in before/after.
- ``record_position_correction`` roots one ORIGIN edge + emits ``POSITION.CORRECTION`` (EVT-172) for
an
  as-known restatement; ``restatement_reason`` (TR-08) lands on the canonical ``justification``
  field.
...
only. ``audit/service.py`` is **FROZEN** — ``POSITION.*`` are caller-side ``event_type`` strings
only.
"""
...
#: ``data_source`` provenance for governed position records (the shared per-tenant MANUAL root).
...
#: ``source_module`` for every position audit event.
SOURCE_MODULE = "position"


@dataclass(frozen=True)
class PositionActor:
    """Actor/correlation context threaded into every position audit emission (BR-16 ready)."""
...
def _origin_edge(session: Session, *, entity: Any, actor: PositionActor) -> None:
    ...
    record_lineage(
        session,
        source=source,
        target_entity_type=ENTITY_POSITION,
...
def record_position_correction(
    session: Session,
    *,
    entity: Any,
