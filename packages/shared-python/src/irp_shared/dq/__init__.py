...
(``not_null``, ``allowed_values``, ``range``, ``completeness``), ``run_quality_check`` (evaluate +
persist + audit), and ``assert_passed_quality_checks`` — the no-silent-failure gate called by the
P1A-4 ingestion finalize (``stage_upload``). *(Corrected at DATA-1: this line had still called the
ingestion caller "future" long after it shipped.)*
