...
class CalculationRun(PrimaryKeyMixin, TenantMixin, ImmutableAppendOnlyMixin, Base):
    ...
    # was scoped to — stamped at creation, never mutated. A WITHIN-TENANT scope label, NOT a security
    # boundary (tenant isolation stays the RLS tenant_id policy). NULL = root not recorded (a
    # snapshot-consume-rooted or pre-0046 run) → honestly unresolvable by the Class-C entity reads.
