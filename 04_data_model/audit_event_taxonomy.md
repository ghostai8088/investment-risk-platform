...
| `RISK` | `RISK.SENSITIVITY_CREATE`, `RISK.FACTOR_EXPOSURE_CREATE`, `RISK.COVARIANCE_CREATE` | **RESERVED — NOT minted (P3-1 implementation, 2026-06-30, OD-P3-1-H; P3-3 implementation, OD-P3-3-K; P3-4 implementation, 2026-07-07, OD-P3-4-L).** The would-be per-result risk-audit family at the **EVT-220 block** (the next domain decade after EXPOSURE/EVT-210; final R-07 index assigned only at activation). **NOT used in P3-1:** the `sensitivity_result` rows (ENT-028) are **run-tracked + lineaged** — metadata of the already-audited `calculation_run` (`CALC.RUN_CREATE`/`CALC.RUN_STATUS_CHANGE`) + the model-registry `MODEL.REGISTER`/`MODEL.VERSION` for the bound model_version — so P3-1 mints **no** `RISK.*` code (the `EXPOSURE.*`/EVT-210 precedent). `RISK.SENSITIVITY_CREATE` is reserved **only** for a possible future granular per-result audit, **if later adopted and documented**. **P3-3 reserves `RISK.FACTOR_EXPOSURE_CREATE` the same way** — the `factor_exposure_result` rows are run-tracked + lineaged (the run reuses `CALC.RUN_CREATE`/`CALC.RUN_STATUS_CHANGE`; the model registration `MODEL.REGISTER`/`MODEL.VERSION`); NO `RISK.*` code is emitted. **P3-4 reserves `RISK.COVARIANCE_CREATE` identically** — the `covariance_result` rows (ENT-051) are run-tracked + lineaged; zero `RISK.*` events emitted (test-asserted). `audit/service.py` stays **FROZEN**. |
