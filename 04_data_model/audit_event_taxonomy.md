...
| `RISK` | `RISK.SENSITIVITY_CREATE` | **RESERVED — NOT minted (P3-1 implementation, 2026-06-30; OD-P3-1-H).** The would-be per-result risk-audit family at the **EVT-220 block** (the next domain decade after EXPOSURE/EVT-210; final R-07 index assigned only at activation). **NOT used in P3-1:** the `sensitivity_result` rows (ENT-028) are **run-tracked + lineaged** — metadata of the already-audited `calculation_run` (`CALC.RUN_CREATE`/`CALC.RUN_STATUS_CHANGE`) + the model-registry `MODEL.REGISTER`/`MODEL.VERSION` for the bound model_version — so P3-1 mints **no** `RISK.*` code (the `EXPOSURE.*`/EVT-210 precedent). `RISK.SENSITIVITY_CREATE` is reserved **only** for a possible future granular per-result audit, **if later adopted and documented**. `audit/service.py` stays **FROZEN**. |
