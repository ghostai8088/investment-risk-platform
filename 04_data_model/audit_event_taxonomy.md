...
| `MODEL` | `MODEL.REGISTER`, `.VERSION`, `.VALIDATE`, `.TIER_ASSIGN`, `.APPROVE`, `.RESTRICT`, `.RETIRE` | Model governance (EVT-050…). `MODEL.REGISTER`/`MODEL.VERSION` **activated in P1A-2** (model/version inventory; assumption/limitation writes fold into `MODEL.VERSION`, no per-row event); **`.VALIDATE` activated in VW-1** (2026-07-14 — one event per `model_validation` record, `after_value` = outcome + validation_type + finding/evidence counts + next_review_due, DC-2; this taxonomy row IS the activation record, the P3-2 precedent); **`.TIER_ASSIGN` MINTED in MG-1** (2026-07-15 — a NEW code, not a reserved-code activation: no existing or reserved code fits tier assignment (`MODEL.APPROVE` is earmarked for the Tier-1 H-02 approval step; no `MODEL.UPDATE` exists); the mint is governed by the ratified MG-1 record (OD-MG-1-C) — this taxonomy row IS the mint record. One event per tier assignment via the 2L `assign_model_tier` verb (gated `model.validate`); caller-side string constant — `audit/service.py` FROZEN, untouched; `before_value` = `{tier}`, `after_value` = `{tier, materiality_rating, complexity_rating, rationale}` — **the event payload is the two RATINGS' durable home** (OQ-MG-1-1 sub-fork (i): the `model` head stores only the derived `tier`; NO new columns, NO migration); an identical re-assignment is a no-op — no version bump, no event); `.APPROVE/.RESTRICT/.RETIRE` stay **reserved** for the P7 approval/restricted-use/retirement legs (the Tier-1 H-02 approval step rides tiering, REQ-MDG-002/003). |
