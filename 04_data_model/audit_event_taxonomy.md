...
| `PORTFOLIO` | `PORTFOLIO.CREATE`, `.UPDATE`, `.STATUS_CHANGE` | **RESERVED (P1C-0 ratification, 2026-06-23; R-07) — NOT yet emitted (taxonomy reservation only; no code change this turn).** The first **domain** event family (beyond reference/rails), for `portfolio` hierarchy governed CRUD (CAP-1/BC-01), reserved at the **EVT-150 block**: `PORTFOLIO.CREATE`=EVT-150, `PORTFOLIO.UPDATE`=EVT-151, `PORTFOLIO.STATUS_CHANGE`=EVT-152 — the head of the P1C domain corridor (TRANSACTION / POSITION / VALUATION take successive ~10-wide blocks EVT-160/170/180). EVT-144–149 are left as REFERENCE headroom; the established ~10-wide-per-category convention (EXPORT 100 / CONFIG 110 / ADMIN 120 / AGENT 130 / REFERENCE 140) places the P1C domain corridor at the **EVT-150 decade** — EVT-150 is the R-07 index assigned at this P1C-0 ratification (the source plan reserved "EVT-144+ free, index assigned by R-07"). **Activation is caller-side in P1C-1** via a new `irp_shared/portfolio/events.py` + thin `record_portfolio_create`/`record_portfolio_update` mirroring `record_reference_*` — `audit/service.py` stays **FROZEN**. `before/after` = DC-2 metadata only (code/name/node_type/parent link); per-tenant chain (PROPRIETARY, no SYSTEM chain). For P1C-1 a `status` flip rides on `PORTFOLIO.UPDATE` (the P1B `is_active`/status precedent), so `PORTFOLIO.STATUS_CHANGE` is **reserved-but-not-required** in P1C-1 (held for a future governed portfolio lifecycle if needed). |
