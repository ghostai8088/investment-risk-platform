...
> **P3-7 realization note (ex-ante active risk / tracking error):** **`active_risk_result` is REALIZED** (migration `0030_active_risk`) as the SECOND physical table under ENT-027 (its THIRD realization — VAR-HS-1 reused `var_result`, adding no table) — **IA TRUE append-only** (in `APPEND_ONLY_TABLES`; `irp_prevent_mutation` P0001 trigger + ORM guard; a re-run = a NEW run + new rows); symmetric tenant-scoped RLS (NEVER hybrid). The SECOND **derived-of-derived** governed number: its `ACTIVE_RISK_INPUT` snapshot pins the IMMUTABLE result rows of TWO upstream governed runs (`COMPONENT_KIND_FACTOR_EXPOSURE` + `COMPONENT_KIND_COVARIANCE` — the IA-row pin flavor) + the covariance `COMPONENT_KIND_FACTOR` EV definitions (the currency→factor map) + the captured `COMPONENT_KIND_BENCHMARK` membership (the per-row immutable FR-version flavor — a later vendor **supersede/correction** of a constituent closes it out and inserts a successor, invisible to the pin, **TR-09**), so a later upstream RE-RUN **or benchmark restatement** leaves a pinned tracking error INVARIANT (test-proven). The model carries NO numeric parameter — the registered `code_version` alone is the version identity (OD-P3-7-D); the active weights `wₐ = w_p − w_b` are the binder's own pure arithmetic over the pinned content (no live read).
