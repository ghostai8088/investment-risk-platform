...
> history. Read this first, then `10_delivery_backlog/delivery_roadmap.md` (the operative sequence) + `bt_1_decision_record.md` (the latest record), `next_actions.md`, and `claude_operating_instructions.md`. **As of HEAD
> `868f892` (merge of PR #5 = `e7b615d`, BT-1 VaR backtesting, ENT-055 + migration 0033, Wave-2 slice 3) / CI green; refreshed 2026-07-11.** Values that
...
- **Branch `bt-1-closeout` — the BT-1 closeout PR** (pending gated commit+push): `bt_1_decision_record.md` Part 6 (14 review findings / 13 folded + 2 deferred-with-reasons: the P3-8/BT-1 return-shape adjudication dedup — tipping-point rule, 2nd of 3 consumers; a pre-existing concurrent-registration race shared by all six family registrars) + this docs refresh (roadmap/phase-ledger/current_state/next_actions). Docs-only — NO migration/permission/audit change.
...
findings/13 folded. Next: THIS closeout PR (branch `bt-1-closeout` — Part 6 dispositions + docs refresh), then
PA-0 planning (private-asset foundations, Wave-2 slice 4) on explicit direction.**
Prior state: P3-0 … P3-5 + P3-C1 + FE-1 + TC-1 + VAR-HS-1 + P3-C2 + TD-1 + P2-7 + P3-7 + P3-C3 + PM-1 + P3-8 + BT-1 all complete and CI-green.
