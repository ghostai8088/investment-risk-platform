...
> history. Read this first, then `10_delivery_backlog/delivery_roadmap.md` (the operative sequence) + `p3_8_decision_record.md` (the latest record), `next_actions.md`, and `claude_operating_instructions.md`. **As of HEAD
> `d769f59` (merge of PR #2 = `86ef3ec`, P3-8 ex-post benchmark-relative, ENT-054 + migration 0032, Wave-2 slice 2) / CI #142 green; refreshed 2026-07-10.** Values that
...
- **Branch `p3-8-cleanup` — the P3-8 cleanup+closeout PR** (pending gated commit+push): (a) the THREE dedup folds reactivated by the clean-code standing bar (proof-of-concept build — cleanliness is a deliverable): `compound_returns` now DELEGATES to PM-1's `link_periods` (one compounding implementation; the exact-linkage coupling is structural), the shared `perf/guards.py::assert_portfolio_in_tenant` (the P3-5 cross-tenant-FK guard lives ONCE, parameterized by each binder's error class), and `bootstrap._register_perf_model` (one code_version-only registrar core; both public registrars delegate); (b) this closeout-docs refresh. Behavior-preserving — NO migration/permission/audit change. Still deferred: the `_reresolve_content` parse-hardening in snapshot verify (pre-existing P3-4 pattern, unreachable via the trusted builder; recorded in `p3_8_decision_record.md`).
...
everyone (P3-8 onward).** **PM-1 (Wave-2 slice 1) DONE — `b2445c7` + `f5e16b6`, CI #137 green; the SEVENTH
governed number, FIRST non-risk (the `perf` family). P3-8 (Wave-2 slice 2) DONE — planning `4880b36` via PR #1,
IMPLEMENTATION `86ef3ec` merged via PR #2 (`d769f59`), CI #142 green; the EIGHTH governed number (ex-post
benchmark-relative AR/TD/TE/IR, ENT-054, migration 0032); FULL 4-finder local review, 5 folds. Next: the P3-8
cleanup+closeout PR (branch `p3-8-cleanup` — the 3 dedup folds + this docs refresh), then BT-1 planning (VaR
backtesting, Wave-2 slice 3) on explicit direction.**
Prior state: P3-0 … P3-5 + P3-C1 + FE-1 + TC-1 + VAR-HS-1 + P3-C2 + TD-1 + P2-7 + P3-7 + P3-C3 + PM-1 + P3-8 all complete and CI-green.
...
- `git log -1 --oneline` and `git status --short` — confirm main HEAD (≥ `d769f59`) and whether the `p3-8-cleanup` branch (dedup folds + this closeout refresh) was pushed/merged.
- Latest CI conclusion for the current HEAD (GitHub Actions; `gh` CLI is NOT installed — the public repo REST API answers unauthenticated, 60 req/hr; `86ef3ec` = run #142 = success at this refresh). **Branch protection binds everyone — implementation lands ONLY via a PR the USER opens+merges; Claude pushes the branch and never touches the token.**
- `git remote -v` — origin is HTTPS (`https://github.com/ghostai8088/…`; flipped from SSH at P3-C3 — port 22 blocked).
- Migration head is `0032_benchmark_relative` (P3-8 / `86ef3ec`); the next migration lands ONLY at the next separately-approved implementation slice.
