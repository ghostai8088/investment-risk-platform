...
> history. Read this first, then `10_delivery_backlog/delivery_roadmap.md` (the operative sequence) + `td_1_decision_record.md` + `p2_7_decision_record.md` (the latest records), `next_actions.md`, and `claude_operating_instructions.md`. **As of HEAD
> `4534a38` / CI green (TD-1 test-data realism audit + follow-up; refreshed 2026-07-09).** Values that
...
- **Local == origin:** yes (0 ahead / 0 behind); only this TD-1 closeout memory refresh is uncommitted.
- **Latest CI:** **GREEN** — `4534a38` (TD-1 follow-up) + `ac92e0b` (TD-1 impl) = GitHub Actions success. TD-1 is test-and-docs only (no production/schema change); locally `make check` 968 passed + full-PG exit 0. Prior `6fb1a13` (P3-C2) green; chain #98–TD-1 all green.
- **Migration head:** `0028_var_historical` — **UNCHANGED by P3-C2 AND TD-1** (TD-1 touched only test files + docs; P3-C2 was type-decorator-only, `PreciseDecimal` renders `NUMERIC(p,s)` byte-identically so `alembic check` stays a no-op). VAR-HS-1 (`29ae31b`) was the last migration: `0027_run_failure_reason` → `0028_var_historical` (`var_result.z_score`/`sigma`/`covariance_run_id` nullable, metric-conditional `ck_var_result_parametric_not_null` CHECK; DESTRUCTIVE RLS-safe downgrade). Drift-clean. **Next migration = P2-7's `0029` at P2-7 implementation.**
...
- **This TD-1 closeout memory refresh only** (Tier 0 — docs-only): the project-memory files + the roadmap's Wave-1 slice-3.5 DONE marker advanced to `4534a38`/CI-green.

## Current active gate
**P3-0 … P3-5 + P3-C1 + FE-1 + TC-1 + VAR-HS-1 + P3-C2 + TD-1 are ALL COMPLETE and CI-green; P2-7 is PLANNED +
ratified (not yet implemented); the RATIFIED `delivery_roadmap.md` (Wave 1) is the operative sequence.** Most
recent: **TD-1** (`ac92e0b` + follow-up `4534a38`, CI-green; plan `2569151`) closed Wave-1 slice **3.5** — a
test-data-realism hygiene insertion (a new user standing rule, 2026-07-09: fixtures economically plausible by
default; extremes only in labeled boundary tests). Retrospective audit + remediation of 8 implausible test
fixtures across factor-return/covariance/VaR/sensitivity/exposure suites (the base market-value fixtures were
already plausible); test-and-docs only, NO production/schema change; 4 independent finder passes (they corrected
2 author errors + caught 2 same-class completeness misses); the three-bucket rule + per-domain bands recorded in
`08_testing_qa/test_data_realism.md` + a standing review-angle. Before it, **P3-C2** (`6fb1a13`) closed slice 3
and **P2-7** (`04c4135`) is planned (ENT-052 benchmark price/level capture; migration `0029`). **Next per the
roadmap: P2-7 IMPLEMENTATION on explicit direction** (build contract `p2_7_implementation_plan.md`; Opus 4.8/high
— the novel design is settled, the build is templated on `factor_return`). Model/effort recommendation standing
rule (2026-07-08): append a
...
- `git log -1 --oneline` and `git status --short` — confirm HEAD (≥ `4534a38`) and whether this TD-1 closeout memory refresh was committed.
