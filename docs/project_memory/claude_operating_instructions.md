...
> **As of 2026-06-25.** How Claude Code works on this project. Read with `current_state.md`, `next_actions.md`,
...
- Migrations sequential (head `0015_valuation`; P1C-5 holdings views shipped read-only with no migration and P1C-6 shipped the synthetic dataset as a never-auto-run seed module — both with NO migration; the next `0016` lands only if a later slice persists a new entity); `alembic check` is a drift gate
