...
**P1C closeout + P2 planning + governance ratification + P2-1 `dataset_snapshot` + P2-2 `fx_rate` IMPLEMENTATIONS are DONE (see the P2 section below); next: P2-3 PLANNING ONLY (`calculation_run` wiring + basic exposure), on explicit approval.**
...
### P2 — Reproducibility foundation, market & private data, risk analytics — **P2-1 + P2-2 DELIVERED (P2-3 exposure planning next)**
...
- **NEXT: P2-3 PLANNING** (`calculation_run` wiring + basic exposure — `exposure_aggregate` as IA append-only, run-bound + snapshot-gated; the first governed derived number, consuming the P2-1 snapshot + the P2-2 `fx_rate`; AD-014/FW-RUN/TR-15), on explicit approval. Then P2-3 implementation (separate approval), P2-4…P2-6, then P3+.
