...
### P1B — Security Master & Reference Data — **DELIVERED (P1B-1..P1B-4 done & CI-green; P1B-5 conditional/deferred; closeout + P1C-0 ratification done; P1C-1 + P1C-2 + P1C-3 + P1C-4 + P1C-5 CLOSED; REQ-PPM-003 Done)**
...
- **P1C-6** deterministic synthetic dataset (synthetic reference seed pack + portfolio hierarchy + transactions/positions/valuations; `uuid5` IDs + fixed timestamps; labeled never-auto-run seed module; no real/sensitive data) — **NEXT (planning only, on approval).** Then P2+ — **FUTURE** (each separately planned + approved).
