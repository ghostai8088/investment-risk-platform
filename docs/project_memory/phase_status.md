...
| **P1B-0** Decision record + plan | DONE (committed); **ratifications recorded** (commit pending) | `dbed93e` (decision record + plan); ratifications in working tree (decision log, requirements, canonical model, temporal, audit taxonomy, entitlement model, project_memory) | green (run 27969793585) | OD-P1B-A…J resolved + **ratified into governance**: AD-013-R1, REQ-SMR-005 + CAP-2.5 re-partition, ENT annotations, `REFERENCE.*` reserved, reference permissions | Audit `REFERENCE.*` codes + entitlement bootstrap **code** minted in P1B build slices (not at P1B-0) |
