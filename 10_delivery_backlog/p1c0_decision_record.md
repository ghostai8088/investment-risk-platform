...
| Status | **Draft for Review (P1C-0, 2026-06-23)** — twelve P1C open decisions resolved as recommendations pending approval. Ratification into the governance source-of-truth (the proposed **AD-017** in `11_decision_log/architecture_decision_log.md`; REQ-PPM-001..004 annotations + status in `02_requirements/`; the `PORTFOLIO.*`/`TRANSACTION.*`/`POSITION.*`/`VALUATION.*` reservation in `04_data_model/audit_event_taxonomy.md`; ENT-010..013 annotations in `04_data_model/canonical_data_model_standard.md` + temporal §2A; portfolio/position/valuation/transaction permissions in `06_security/entitlement_sod_model.md`) is a **separate governed step after approval**, mirroring P1B-0's plan→ratification split (`4fae26b`). **Two-step audit/entitlement governance:** at ratification, the new domain-event **family + a reserved EVT index block** (one ~10-wide block per category, like REFERENCE's EVT-140 block) and the new **permission codes** (`PORTFOLIO.*`/`TRANSACTION.*`/`POSITION.*`/`VALUATION.*`; `transaction.*`/`position.edit`/`valuation.*`) are **recorded/reserved** in the taxonomy + entitlement model (R-07); the specific codes + entitlement bootstrap **code** are then **activated/minted caller-side inside the P1C build slices** (never at P1C-0; `audit/service.py` stays FROZEN). |
