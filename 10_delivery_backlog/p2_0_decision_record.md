...
- **Entitlement (R-07)** — **wire `exposure.aggregate.run`**; mint additively `snapshot.view`/`.create`, `marketdata.view`/`.ingest`, `exposure.view`; **maker = the existing `data_steward`** (no new role minted — vendor-license isolation is enforced by tenant-scoped RLS, not a role); reconcile `marketdata.ingest` vs the existing `data.upload` (recommend `marketdata.ingest` = governed canonical-write, distinct from raw staging upload); `auditor_3l` excluded; deny-by-default.
