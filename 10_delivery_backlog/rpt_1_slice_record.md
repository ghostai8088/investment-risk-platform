...
|---|---|---|
| **The SQLite FK gap — PAID at FK-1 (2026-08-08)** | Was: the shared unit engine leaves `PRAGMA foreign_keys` OFF; measured 115 across 12 suites, 12 paid here, 103 carried. **FK-1 re-measured before building and the carried number was STALE: 151 across 14 suites** — two backend endpoint suites and `test_benchmark_series` had joined, because suites written after this census inherited the blind engine. Enforcement now lives in `make_engine` itself (dialect-guarded), all 151 fixtures seed genuine parents, this suite's interim local listener is RETIRED, and the property is pinned by `test_db_foreign_keys.py` (negative control matched on the FK message, positive control, factory-property test; 4-mutant battery). Full account: `fk_1_slice_record.md` | PAID |
