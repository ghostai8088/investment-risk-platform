...
| | |
|---|---|
| **Status** | **✅ CLOSED 2026-07-24 — implemented, reviewed, merged. PR #115** (merge commit `96679e2`, into `main`), CI green all 6 checks; **NO migration** (code-only — `0002_entitlement_seed` re-seeds `limit.approve` from the live bootstrap catalog; head stays `0051_breach_action`). Realizes the genesis-reserved `LIMIT.APPROVE` (EVT-060) as the DRAFT→ACTIVE maker-checker gate; **NOT a governed number** (counts UNCHANGED **23/38/109**). Gate outcome: **OQ-MG-3-5=A** (gate limit CHANGES too, not just creation — full REQ-LIM-001: a material governing-field edit to a live limit auto-demotes to DRAFT for a non-editor re-approval); **OQ-MG-3-2=A** (person-level SoD, `limit.approve` on the same `risk_manager_2l`); **OQ-MG-3-3=A** (standalone SoD check). Pre-ratification verifier folded 3 blocking mechanical holes (B-1 no-migration / B-2 stale-read-under-lock / B-3 create-side bypass) + the B#1 change-scope gap ratified into scope as OQ-5=A. **4-finder adversarial review: 1 HIGH (THREE finders independently converged — a solo 2L could bypass the change-gate via suspend→edit→resume OR a no-op `status=` slipped alongside a governing edit; fixed: demote on a real governing change to ANY non-DRAFT limit + refuse status/governing combos) + 4 MED** (update_limit TOCTOU → fresh-status scalar `FOR UPDATE`; no-op-resave spurious demote → value-compare; SoD excluded only the last editor → the approver ∉ `{created_by, updated_by}` SET; audit `checked_makers`) + LOWs — all folded; `make check` + clean full PG suite (1912 passed) green. **The last Wave-11 slice — WAVE 11 FUNCTIONALLY COMPLETE; NEXT = the mandatory Wave-11 close review.** MED-3 (SoD `actor_id` canonicalization) NOT folded — a forward-gate carry for the future limit-API auth boundary (the SSO-1 lesson). Original ratification below. |
