...
| 3.5 | **TD-1 — test-data realism audit** (INSERTED 2026-07-09; ratified) | A hygiene insertion (Part 4 rule 3) ahead of P2-7 implementation: retrospectively audit + remediate existing economic-value fixtures to the new fixture-realism standing rule (three-bucket classify-then-fix; test-and-example-only diff fence; re-derive any dependent assertions). `td_1_decision_record.md` OD-TD-1-A…F; OQ-TD-1-1…6 ratified. Escape hatch: split into TD-2 if derived-number re-derivation exceeds one slice. | M/L | none |
| 4 | **P2-7 — benchmark price/level capture** ✅ **PLANNED** (`04c4135`, CI green; OQ-P2-7-1…8 ratified) | The captured-input slice (`benchmark_level`/`benchmark_return`, NET-NEW **ENT-052**) that unblocks every return-based benchmark-relative analytic. Follows the P2 captured-data pattern (FR/bitemporal; no run/model/snapshot binding). Implementation follows TD-1 (user-directed order 2026-07-09). | M | none |
