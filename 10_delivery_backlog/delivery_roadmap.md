...
| 5 | **P3-7 — benchmark-relative analytics** | Ex-ante active risk / tracking error `TE = √(wₐᵀΣwₐ)` over the existing factor-exposure + covariance engine and the captured benchmark membership — the P3 plan's final **ex-ante** analytic leg. The **ex-post** leg (realized TE / active return / IR) is DEFERRED on the portfolio-return prerequisite (OD-G). | M/L | slice 4 |
