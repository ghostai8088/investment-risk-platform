...
| | |
|---|---|
| **Status** | **CLOSED 2026-07-15** — planning `da258ff` merged via **PR #38** = `26d89b4`; impl `3cb9844` merged via **PR #39** = `4035f10`, CI green; migration `0040_var_estimate_age`. *(Stamped at the Wave-5 close — BT-2 was the one Wave-5 slice with no closeout PR of its own and fell through the gap between the VW-1 and ES-1 closeouts. The THIRD consecutive close to find a missing closure stamp; see the close review's process finding.)* **RATIFIED 2026-07-15** — OQ-BT-2-1…6 approved as recommended (user: "Proceed"), incl. OQ-BT-2-3's sub-fork **(i) the regression-span-end anchor** (the estimation snapshot's `as_of_valuation_date`, live-read via the pinned `input_snapshot_id`). Implementation gated separately behind `bt_2_implementation_plan.md`. |
...
DISCHARGED; PA-4 Part-6 deferral 1 DISCHARGED; the estimate-staleness register item ~~CLOSED~~ — see the correction below
...
> **⚠️ The estimate-staleness register item is NOT closed — the claim was an OVERCLAIM, corrected at the Wave-5 close (code-verified).** The minted item had THREE clauses; BT-2+VW-1 paid ONE. ✅ *records the estimate age at total-VaR time* — PAID (migration `0040` + the echo). ⚠️ *forces a promoted REGRESSION weight to cite a recent estimate* — paid **only on the total-VaR-v2 bind path**: `promote_proxy_weight_estimate` has NO age check at all, and `var_service` sets `max_estimate_age_days = None` off the total family, so the SAME promoted weight drives `risk.factor_exposure.proxy` → plain VaR / **plain ES** / active risk / scenario entirely ungated. ❌ *expires one* — UNPAID: nothing time-based closes a `proxy_mapping`; `valid_to` moves only on an explicit human supersede. The item's own headline — *"a years-stale σ_e drives current total VaR indefinitely and silently"* — is now false for total-v2 binds and **still true everywhere else**. **RE-OPENED as "estimate staleness — residual", TIPPED** (the machinery exists; the residual is cheap).
>
> **Also recorded at the Wave-5 close — a ratified obligation BT-2 dropped:** OD-G (via OQ-BT-2-6) required *"BT-1 deferral B … verify MD-H1's `resolve_or_insert` generalization already paid it and **record the discharge**"*, and the plan repeated it. Part 6 never mentioned it. **The discharge is hereby RECORDED, and the underlying state is fine** (code-verified at the close): `model/service.py` wraps the racy INSERT in `begin_nested()` savepoints with `IntegrityError` recovery, and all three Wave-5 registrars (`parametric_total` v2, `parametric_es`, `parametric_es_total`) route through `resolve_or_register_version` — the pattern HELD. The defect was the unrecorded discharge, not the code.

**BT-2 CLOSED 2026-07-15** — planning `da258ff` via **PR #38** = `26d89b4`; impl `3cb9844` via **PR #39** =
`4035f10`, CI green; head `0040_var_estimate_age`. Stamped at the Wave-5 close.
close).
