...
| 1 | **CON-1 — concentration, the 23rd governed number family (REQ-CRD-003's concentration half)** — ***AMENDED IN PLACE at the CON-1 ratification (2026-07-30, OQ-CON-1-1…28; the Part 6b clause): the row below records the RATIFIED shape where it diverges from this row's original text.*** | A NEW result family (new table + migration `0057`; ONE new snapshot PURPOSE with FOUR pinned shapes). ***Dropped at ratification:** the `ck_schedule_model_version_by_family` amendment + `FAMILY_REGISTRY` entry (schedulability deferred, OQ-CON-1-17) and the in-slice `_METRIC_MAP` registration (REVERSED to LIM-2, OQ-CON-1-15 — the unbased-limit window; REQ-CRD-003 advances to "produced, bindable at LIM-2", not Done).* ***Reversed:** "latest COMPLETED exposure run" — the binder takes an EXPLICIT run selection; latest survives only as the API convenience default (Part 6b item 5).* **DESCOPED denominator (user-ratified 2026-07-30): ONE share, `share_invested_long`, `denominator_basis` vocabulary; regulatory-shaped limits refused until LIM-2's basis machinery.** Registered model_version WITH declared parameters; the scheme-qualified dimension identity ratifies with the LIM-2 selector as the named acceptance constraint. Rule-7 reads + **minimal FE read KEPT in-slice (decision point 8)**; PG-tier pins on non-String filters. Immediate-issuer grain; ultimate-parent rollup deferred (trigger recorded in the CON-1 record Part 6b item 4). Three-code R-07 mint (auditor_3l excluded from issuer-identity reads). Counts 25/40/133 → 26/41/136. | M/L |
