...
| THR-05 | File upload | Malicious file / malware / oversized (T/DoS) | High | BC-16 Integration | Type/size validation, sandboxed parsing, AV scan, quarantine | Implemented-skeleton (P1A-4: CSV-only allowlist, 10 MiB cap counted while reading, explicit-encoding decode, std-lib parse); **AV scan deferred** (OD-042, no-op `scan_status` seam — later-hardening) |
| THR-06 | File upload | CSV/Excel formula injection, path traversal (T) | Medium | BC-16 | No formula execution, safe parsers, filename sanitization, ACL | Implemented-skeleton (P1A-4: per-cell formula-injection neutralization `= + - @ TAB CR`, std-lib `csv` parse, filename sanitization to basename) |
