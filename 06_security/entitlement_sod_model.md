...
| ROLE-ADM | Administrator | — | User/role admin; **cannot** approve own entitlement requests or edit audit. **REALIZED as the `tenant_admin` template at ONBOARD-1a (2026-08-09; empty until 1b mints its verbs); the cannot-approve-own clause becomes OQ-ONB-9A's four-eyes lifecycle at 1b (SOD-04, CTRL-025)** |
...
| commitment / capital_call / distribution (ENT-015/016) | `commitment.view`, `commitment.edit`, `commitment.record` | **MINTED IN CC-1 (2026-07-20; OD-CC-1-B, ratified as the Tier-3 fork; `entitlement/bootstrap.py`).** THREE codes — the family spans BOTH temporal classes and the **verb shape is doctrine**: **`commitment.edit`** is the FR maker (a commitment is captured/superseded/corrected — `.edit` not `.record`, the `position.edit`/`valuation.edit` rule) and **`commitment.record`** is the IA maker (a call/distribution is *recorded*, never edited — the `transaction.record` rule; a reversal is itself an appended record), so ONE mixed-shape verb was rejected; **`commitment.view`** reads all three tables. Grants: **both makers = `data_steward` + `platform_admin` ONLY** (identical holder sets — the third code adds NO SoD surface); `commitment.view` → `risk_analyst_1l`, `risk_manager_2l`, **`data_steward`** (+ `platform_admin`). **`auditor_3l` excluded from all three** — captured private-capital INPUTS take the operational client-data SoD (the `portfolio`/`transaction`/`valuation`/`marketdata` exclusion precedent), NOT the governed-output oversight inclusion (`exposure.view`/`risk.view`/`perf.view`); the auditor will read the governed CC-2 projection outputs instead. Deny-by-default (ENT-P-01); pinned both directions by `test_commitment_permissions_grants_as_ratified` + the per-verb endpoint parity (`test_private_capital_endpoint.py`: edit cannot record, record cannot edit, view cannot write). |
| pacing_projection_result (ENT-059) | `pacing.run`, `pacing.view` | **MINTED IN CC-2 (2026-07-20; OD-CC-2-A, ratified as the Tier-3 fork; `entitlement/bootstrap.py`).** TWO codes — a GOVERNED-OUTPUT family (the SEVENTEENTH governed number), so it takes the governed-number verb shape, NOT the captured-input CC-1 shape: **`pacing.run`** is the maker (run a commitment-pacing projection — the `risk.run`/`perf.run` rule) and **`pacing.view`** reads the projection results. Grants: **`pacing.run` = `data_steward` + `risk_analyst_1l` + `platform_admin`** (the governed-run maker set, the `risk.run`/`perf.run` precedent; `auditor_3l` excluded from the maker); **`pacing.view` = `data_steward`, `risk_analyst_1l`, `risk_manager_2l`, `auditor_3l`** (+ `platform_admin`). **`auditor_3l` INCLUDED in `.view`** — the DECISIVE contrast with CC-1's `commitment.view`: a governed OUTPUT takes the oversight-inclusion SoD (`exposure.view`/`risk.view`/`perf.view`), where the CC-1 captured INPUT excluded the auditor; the projection IS the number the auditor was promised at CC-1. The registrar reuses `model.inventory.register` (no new register verb). Deny-by-default (ENT-P-01); pinned both directions by `test_pacing_permissions_grants_as_ratified` (the auditor-view-parity leg asserted) + the endpoint parity (`test_pacing_endpoint.py`: view cannot run/register, a stranger denied everywhere). |
| classification_scheme/node/assignment (ENT-066/067/068) | `reference.classification.view`, `reference.classification_assignment.view`, `reference.classification.edit` | **MINTED IN REF-1 (2026-07-29; OQ-REF-1-26) — catalog row ADDED at the CON-1 sweep 2026-07-30** (the P1 ledger discipline: REF-1's mint was live in `entitlement/bootstrap.py` and pinned by tests but never recorded HERE — a register omission the seven-ledger sweep caught). THREE codes split by tenancy class: `.view` (hybrid global vocabulary — `auditor_3l` INCLUDED), `.classification_assignment.view` (PROPRIETARY assignments — `auditor_3l` EXCLUDED, the issuer-identity class), `.edit` (the steward maker). |
| concentration_result (ENT-069) | `concentration.run`, `concentration.view`, `concentration.issuer.view` | **MINTED IN CON-1 (2026-07-30; OQ-CON-1-25 v6, user-ratified with the holder sets ENUMERATED in the record and recomputed from `ROLE_TEMPLATES` source).** THREE codes split by WHAT THE READ EXPOSES: **`concentration.run`** = {`platform_admin`, `data_steward`, `risk_analyst_1l`} (the `risk.run`/`pacing.run` maker precedent); **`concentration.view`** = run-set + {`risk_manager_2l`, `auditor_3l`} — the governed-output oversight inclusion, payload structurally FREE of issuer identity (excluded at the service query, not the router); **`concentration.issuer.view`** = `.view` MINUS `auditor_3l` — the ISSUER-dimension detail rows carrying `issuer_id`, consistent with the THREE prior issuer-identity refusals (`reference.issuer.view`/`legal_entity.view`/`classification_assignment.view`). One combined code would have handed the 3L auditor exactly the proprietary-identity read those mints refused — and the per-code `_holders` pins would have PASSED (REF-1's own finding), which is why the route census (`test_concentration_endpoint.py`) additionally asserts the issuer-bearing route demands the `.issuer.view` code BY NAME. Pinned both directions in `test_concentration_kernel.py::TestGovernancePins`. |
| liquidity_result (ENT-071) | `liquidity.view`, `liquidity.run` | **MINTED IN LQ-1 (2026-08-02; OQ-LQ-1-13) — catalog row ADDED at the RPT-2 mint 2026-08-07** (the SAME register-omission class as REF-1's row, caught one slice later this time; the P11 evidence clause had already recorded the miss). TWO codes: **`liquidity.view`** = {`data_steward`, `risk_analyst_1l`, `risk_manager_2l`, `auditor_3l`, `platform_admin`} — the governed-output oversight inclusion (the payload carries no proprietary identity, so no narrower split exists); **`liquidity.run`** = {`data_steward`, `risk_analyst_1l`, `platform_admin`} — maker/admin-only, and STILL UNROUTED (the forward-gate record lives in the catalog comment and in `test_route_permission_census.py::UNROUTED_FORWARD_GATES`). Holder pins added at RPT-2: `test_liquidity_grants_as_ratified` (both directions, the auditor-never-runs leg asserted). |
| report_generation (ENT-072) | `report.generate`, `report.view` | **MINTED IN RPT-2 (2026-08-07; `entitlement/bootstrap.py`). RATIFICATION STATUS — corrected at the pre-merge audit:** the Wave-16 gate asked no permission question and enumerated no holder set, so this row's earlier claim of user ratification was FALSE. The holder sets below are the builder's application of standing doctrine and are carried to the Wave-16 close as an explicit Tier-3 item — every prior mint (OQ-CON-1-25, OQ-LQ-1-13) had its sets enumerated to the user before shipping, and this one has not yet. TWO codes split by VERB CLASS on the auditor line: **`report.view`** = {`data_steward`, `risk_analyst_1l`, `risk_manager_2l`, `auditor_3l`, `platform_admin`} — the rendered report IS the governed numbers with provenance, so it takes the governed-output oversight inclusion (the unbroken `exposure.view` → … → `liquidity.view` chain); **`report.generate`** = {`data_steward`, `risk_analyst_1l`, `platform_admin`} — a WRITE verb (mints a REPORT `calculation_run`, a `REPORT_INPUT` snapshot and an IA ENT-072 row), auditor EXCLUDED (3L observes evidence, never creates it), 2L excluded (generating is running the numbers, not approving them — the `*.run` maker precedent). **SOD-08 is HALF-discharged**: "Generate a report" is minted here; "Approve/publish a board report" is deliberately NOT minted — no publish workflow ships in RPT-2, and minting the checker half without its workflow would be a dead guard (the `liquidity.run` forward-gate lesson, inverted). ROLE-RC "Report Consumer" (§4) remains a business-role description with NO template counterpart — minting a template role is its own Tier-3 act under the same R-07 freeze. Pinned both directions by `test_report_grants_as_ratified`; route→code census in `test_reports_endpoint.py` + the PLATFORM-WIDE census (`test_route_permission_census.py`, built at this mint per P11-as-ratified); endpoint parity: view-cannot-generate, stranger-denied-everywhere, 401 bare, 405 on mutation verbs. |
...
| tenant (ENT-074) + the PLATFORM catalog | `tenant.create` (PLATFORM catalog); `tenant_admin` template (EMPTY in 1a) | **MINTED IN ONBOARD-1a (2026-08-09; ratified `onboard_1_decision_record.md` OQ-ONB-2A/4A/5; migration `0067`).** `tenant.create` is the platform's first **PLATFORM-SCOPE** code and it deliberately does NOT live in `entitlement/bootstrap.py PERMISSIONS` — a code there enters `ALL_CODES` → the `platform_admin` template → every tenant's clones, which composes to "every customer tenant can create tenants" (FIVE independent verifier lanes converged on that in the design's first draft). It lives in `entitlement/platform_catalog.py`, held by **`platform_operator`** — a SYSTEM-only role that is NOT in `ROLE_TEMPLATES` and is NEVER cloned — and by nothing else, `platform_admin` included. Enforced structurally: a disjointness census (`PLATFORM_PERMISSIONS ∩ PERMISSIONS = ∅`), a DB-level census that no CLONED role holds a platform code (asserted AFTER an onboarding, because the constants were never what leaked), and the SYSTEM-router fence (a SYSTEM-tenant principal is refused on every router except `/tenants`, census-pinned against the walked 292-route surface). **`tenant_admin` is the SEVENTH template, minted EMPTY**: the role exists because the onboarding seed grant needs something to grant; its verbs (`user.manage`, `role.assign`, `user.view`, `role.approve`) are ONBOARD-1b's mint and land WITH the routes that enforce them (an exact-exemption test carries the reason and a stale-entry twin fails when 1b lands and the exemption is not deleted). Route census: `POST /tenants` guarded by the ordinary `require_permission` (an inline check would have made the platform's most privileged route the one route the census cannot see). Holder pins + refusal battery: `test_tenancy.py`, `test_tenants_endpoint.py`, mutants N-A1/N-A2/N-C1..C3; the escalation refusal proven LIVE on the deployed stack (`prove_onboarding.sh` arm 3c). Clones: customer tenants receive the four business templates + `tenant_admin` — **NOT `ops`** (its only code drives the BYPASSRLS ops CLI; no HTTP consumer) and **NOT `platform_admin`** (its everything-bundle collapses SOD-02/SOD-03/MG-3 inside a customer tenant); ratified OQ-ONB-6. |
| entitlement_request (ENT-075) + the tenant_admin verbs | `user.manage`, `role.assign`, `user.view`, `role.approve` | **MINTED IN ONBOARD-1b (2026-08-09; ratified `onboard_1_decision_record.md` OQ-ONB-5/9A; migration `0068` carries the `DELIVERS` tuple; catalog row added at the review fold — the third slice running to pay this ledger at review rather than build, now a pattern worth naming).** FOUR codes filling the `tenant_admin` template 1a minted EMPTY (the stale-entry twin forced the exemption's deletion in the same commit). Holders: **`tenant_admin` + `platform_admin` ONLY** — no business role administers users. **`user.view` EXCLUDES `auditor_3l`**, ratified OQ-ONB-5: the roster carries `external_subject` + `display_name`, person-identifying data in the class every proprietary-identity read has withheld from the 3L auditor (`reference.issuer.view`/`legal_entity.view`/`classification_assignment.view`); a redacted auditor roster is deliberately NOT minted (the SOD-08 half-mint precedent — its trigger is a real 3L access-review requirement). **`role.approve` is the SOD-04 checker verb and sits on the SAME role as the maker verbs** — the MG-3 pattern: a tenant's admins are peers, so the gate that means anything is person-level (approver ≠ requester by canonicalized principal id), asserted by its own test; a role-level split would add ceremony without preventing one person holding both. §5C checklist for this mint: rows 1 (holder pin, `test_tenant_admin_grants_as_ratified`), 2 (route census walks both catalogs, 300 routes), 3 (THIS row), 4 (`DELIVERS` in 0068, P17-read), 5 (**EXECUTED as a named backfill IN 0068** — existing tenants' `tenant_admin` clones receive the four verbs, decision recorded in the slice record: one deployment path exists and an admin role silently lacking the verbs its own UI offers is worse than the drift the general rule guards) — ALL FIVE rows executed, none refused. |
| limit/breach/schedule/audit-ops (ENT-031/033/034, ENT-061/062, checkpoint) | `limit.view`, `limit.manage`, `limit.approve`, `breach.view`, `breach.respond`, `breach.review`, `schedule.view`, `schedule.manage`, `ops.audit.verify` | **CENSUS BACKFILL 2026-08-09 (the mechanical ledger census's first run): NINE codes minted across LIM-1/MG-2/MG-3/API-2/API-2b/SCH-1/OPS-H1 (Waves 11–12, each ratified in its own decision record) that PRE-DATE the per-mint SoD-row discipline (begun ~REF-1) and were never recorded here.** Holder sets recomputed from `ROLE_TEMPLATES` source at backfill, not transcribed from memory: `limit.view`/`breach.view`/`schedule.view` = all four business roles + `auditor_3l` (+`platform_admin`) — governed-output oversight inclusion; `limit.manage`/`limit.approve`/`breach.review` = `risk_manager_2l` (+`platform_admin`) — the 2L checker tier (MG-3's maker-set person-level SoD rides `limit.approve`; MG-2's 1L/2L partition rides `breach.respond` vs `.review`, SOD-02); `breach.respond` = `risk_analyst_1l` (+`platform_admin`) — the 1L responder; `schedule.manage` = `data_steward` + `risk_analyst_1l` (+`platform_admin`); `ops.audit.verify` = `ops` (+`platform_admin`) — the chain-verification CLI verb, no HTTP surface. Enforcement was never in doubt (each slice shipped its parity tests); the REGISTER was the gap, which is precisely what the census now fails the build over. |
...
in this document and the discipline lived only in the rule's prose.

| # | Obligation | Mechanical check |
|---|---|---|
| 1 | Holder-set pin test (both directions) | the per-mint `test_*_grants_as_ratified` pattern |
...
reaches that tenant's clones only by an explicit decision. Silence here is how a mint quietly
never arrives for every customer that signed up before it.
...
**The single-admin bootstrap window (ONBOARD-1b, ratified OQ-ONB-9A 2026-08-09).** For SOD-04, four-eyes engages the moment an approver EXISTS: an entitlement-affecting act is born `PENDING` when the tenant has ≥1 currently-valid OTHER admin (at two admins, not three — the ratification's own first fix wrote "≥2 other" and thereby exempted every two-admin tenant, verifier pass 2 finding B3). With NO other admin the act executes directly and is stamped `direct_grant=true` on its `ROLE.GRANT_REQUEST` audit event — the window is bounded by the admin count itself and every use of it is countable evidence, not silence. Scope (the record's enumeration, exactly): grant, revoke (spelled as end-dating), and deactivation of a user CURRENTLY holding `tenant_admin`; a non-admin deactivation is user management and stays direct. The lifecycle store is ENT-075 `entitlement_request` (IA append-only — an approval is an APPENDED resolution row, and "approved by nobody" is unrepresentable by CHECK).
