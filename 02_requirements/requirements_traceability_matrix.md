...
| REQ-MKT-001 | 5.1 | P-RA,P-RM | 1L/2L | P2 | BX-AUD | BX-ENT | BX-LIN | **Y** | CTRL-002/003/018/020 | FW-RUN, DEP-MREG, DEP-MGW, DEP-LIN, CAP-1/3 | In-Progress (P3-5 parametric leg: zero-mean delta-normal 1-day VaR — `var_result` ENT-027 realized, migration `0026`; acceptance discharged for the leg: hand references + numpy cross-check within ε, re-run identical, methodology doc + inventory entry. P3-4 delivered the covariance substrate `0025`. **Historical simulation + Monte-Carlo are user-directed ROADMAP legs (2026-07-07)** — factor-based historical-sim feasible with current data, MC gated on a seeded simulator + revaluation; ES leg deferred; the REQ does NOT close) |
