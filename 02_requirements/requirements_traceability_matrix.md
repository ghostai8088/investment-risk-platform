...
| REQ-PUB-003 | 3.4/3.5 | P-DS,P-RA | Plat | P2 | BX-AUD | BX-ENT | BX-LIN | — | CTRL-029 | FW-TMP, DEP-DQF, CAP-2 | **In-Progress (partial, P2-6 implementation, migration `0021`):** `credit_spread` (ENT-023, P2-5 by value) + **`benchmark` (ENT-009) REALIZED** (P2-6: `benchmark` EV + `benchmark_constituent` FR) — the spread-coverage AND benchmark-membership **"inputs present & as-of"** legs delivered (symmetric RLS (CTRL-029); VENDOR_CURVE + VENDOR_BENCHMARK lineage; `REFERENCE.*`/`MARKET.BENCHMARK_CONSTITUENT_*` audit). **`benchmark_level`/`benchmark_return` REALIZED** (P2-7, migration `0029`, the net-new ENT-052 under the ENT-009 header; captured levels + vendor-published returns; `MARKET.BENCHMARK_LEVEL_*`/`_RETURN_*` audit; VENDOR_BENCHMARK lineage) — the benchmark time-series "inputs present & as-of" leg now delivered. **`rating`** + the full **Coverage test** remain **DEFERRED** — REQ does **NOT** close |
