...
| REQ-PUB-003 | 3.4/3.5 | P-DS,P-RA | Plat | P2 | BX-AUD | BX-ENT | BX-LIN | — | CTRL-029 | FW-TMP, DEP-DQF, CAP-2 | **In-Progress (partial, P2-5 implementation, migration `0020`):** `credit_spread` (ENT-023) shipped **by value** (`curve_type=CREDIT_SPREAD` + `reference_key` over the same `curve`/`curve_point` tables) — the **"inputs present & as-of"** spread-coverage leg delivered (symmetric RLS (CTRL-029); VENDOR_CURVE lineage). **`rating` + `benchmark`** + the full **Coverage test** **DEFERRED** — REQ does **NOT** close |
