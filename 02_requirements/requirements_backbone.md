...
| REQ-LIM-002 | Utilization computation | 10.2 | Measure usage vs limit | Compute utilization from risk results | risk results, limits | Utilization calc | Utilization test | Utilization reproduces and binds source results | **In-Progress (LIM-1 W11):** `evaluate_limit` binds the evaluated `calculation_run` and compares observed metric vs threshold (utilization-in-effect); the reserved ENT-032 `limit_utilization` surface is a Wave-14 LIM-2 slice-gate call *(re-synced at the Wave-14 planning gate)* |
...
| REQ | Title | CAP | Business purpose | Functional | Data | Calc | Test | Acceptance | Status |
|---|---|---|---|---|---|---|---|---|---|
| REQ-ADM-001 | Real SSO / OIDC identity | 17.1 | Enterprise identity | Replace dev header shim with OIDC + MFA | users/principals | — | Auth integration test | Principal comes from OIDC; MFA enforced (AD-007) | **In-Progress (SSO-1 W9 PR #86 real OIDC boundary + FE-3b W10 PR #95 browser PKCE):** the principal-from-OIDC leg is realized; MFA posture rides the IdP; admin surfaces + full RTM-P9 replacement of the dev shim remain *(re-synced at the Wave-14 planning gate, OQ-W14P-7)* |
