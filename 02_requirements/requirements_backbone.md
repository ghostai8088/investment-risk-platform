...
| BX-REPRO | Every calculation result is reproducible from its bound run — **MACHINE-CHECKABLE for three families since REPRO-1 (2026-08-07) — the sweep exists and is proven on a schedule, but no demo or deploy path creates one yet (carry m); an assertion for the other eighteen, coverage enumerated by census** | BR-6, BR-9 | CTRL-018 | FW-RUN |
