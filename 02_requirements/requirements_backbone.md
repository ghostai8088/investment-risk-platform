...
| REQ-PUB-003 | Credit spreads, ratings, benchmarks | 3.4/3.5 | Credit & relative risk inputs | Spread series, ratings, benchmark constituents | `credit_spread`, `rating`, `benchmark` (FR/EV) | — | Coverage test | Inputs present & as-of for the risk engine | **In-Progress (partial, P2-5 implementation, migration `0020`):** `credit_spread` (ENT-023) realized **by value** (`curve_type=CREDIT_SPREAD` + `reference_key` over the same `curve`/`curve_point` tables) — the **"inputs present & as-of"** spread-coverage leg delivered. **`rating` + `benchmark`** + the full **Coverage test** are **DEFERRED** — REQ does **NOT** close |
