...
| Citation rule | Roadmap Part 4 rule 6a, strengthened 2026-07-30: citations enter records only as verbatim quotes with locators, checked by an independent citation lane that sees only the sources. |
...
| Status | CHECKED 2026-09-18 by an independent citation lane (Opus 5): all seven sources re-fetched, 33 of 33 quoted passages verbatim after whitespace normalisation and ASCII folding of typographic quotes and dashes; 13 findings folded (section 6). Nothing below was written from memory. Every quote was copied from a file fetched on the access date. |

## 1. Method, and what failed

Each source was downloaded with `curl` and the text was extracted locally (pypdf for PDFs, tag stripping for HTML). Quotes were copied from the extracted text. Where pypdf split a word with a stray space (for example "Va R", "integr ated", "mo st"), the word was joined, and typographic quotes, apostrophes and dashes were rendered as ASCII. Nothing else was altered (F-7: eleven of the 33 passages carry that ASCII folding, and no words were added, dropped or reordered in any). Page numbers for PDFs are the PDF page index; where the document prints its own page number the printed number is given too.
...
- sec.gov rate-limits repeat fetches: the citation lane's re-fetch of the same PDF got HTTP 403 "SEC.gov | Request Rate Threshold Exceeded" from `curl` on both the `/rules/` and `/files/` paths, and obtained the file (406 pages, 2.7 MB) through the WebFetch fallback. A re-checker may need that route (F-0).
...
Quote S1-d (section 3.6, explanatory text paragraph 74; PDF page 34; added at F-1):
...
Quote S3-c (section II discussion of the VaR tests, the passage on stressed VaR and expected shortfall; PDF pages 92 to 93; added at F-1; footnote markers "301" and "302" dropped):
...
Quote S3-d (section on the Form N-PORT reporting amendments, the paragraph on how the Commission will use reported VaR data; PDF page 202; added at F-12; footnote marker "663" dropped):
...
Quote S6-f (section "PERFORMANCE ATTRIBUTION", printed pages 04 // 05, PDF page 4; added at F-4):
...
| J-CRO-2 | Headline row: total exposure, VaR and ES at governed confidence and horizon with as-of date, tracking error vs benchmark, provenance one click away | S1-a (one-day VaR against end-of-day positions), S3-a (fund VaR vs reference portfolio VaR), S5-a and S5-b (VaR, market exposure), S6-a, S6-b, S6-c (tracking error, VaR at multiple confidence levels, absolute or relative to benchmark), S6-b (click through to underlying data), S3-c and S1-d (expected shortfall / CVaR) | Expected shortfall IS named in two fetched sources (F-1): SEC IC-34084 (S3-c, PDF pages 92-93) defines it and states that funds "may incorporate these methodologies into their derivatives risk management programs" while declining to require it; CESR/10-788 (S1-d, PDF page 34, explanatory text paragraph 74) recommends "CVaR or other methods able to detect the potential impact of low-probability market events" for leveraged arbitrage strategies. Neither describes it as a headline display item. "Provenance one click away" is closest to S6-b's click-through to underlying data. As-of date is not stated as a display item anywhere. |
| J-CRO-3 | Limit posture: in force, breached, near threshold (80 percent), open breaches with owners and due dates | S1-c (documented internal VaR limits), S3-b (VaR test breaches reported), S4-a (d), (e), (f) (risk limit system, compliance, timely remedial action on breaches), S7-c (threshold monitoring, escalation workflows) | Breach owners, response due dates, and the 80 percent near-threshold band are not in any source. Not walkable before UTIL-1 by design. |
| J-CRO-4 | Exposure composition by asset class, currency, sector, factor family; public beside private | S5-b (country and sector exposures, factor decomposition), S6-d (break down by asset class, sector, region, duration, credit quality), S7-a, S7-b, S7-d (exposures by factor, sector, security; public and private on one view) | Currency as a standalone exposure-composition axis on a risk overview is not described in any fetched quote; S6-f (PDF page 4) does name currency among the risk-model factors portfolios have exposure to, beside "currency effect" under attribution (F-4). |
...
J-CRO lines that NO source mentions at all: none of the eight is entirely absent. The parts of lines that no source mentions are: fund ranking by headroom and change since last close (J-CRO-1); the as-of date as a display item (J-CRO-2; expected shortfall itself IS named, S3-c and S1-d, but not as a headline item); breach owners, due dates and the 80 percent band (J-CRO-3); currency as a standalone exposure-composition axis on a risk overview (J-CRO-4, though S6-f names currency among the risk-model factors a portfolio has exposure to); desmoothing and the reported-vs-desmoothed difference (J-CRO-5, the whole point of the line); VaR trend and rolling drawdown charts (J-CRO-7); the sum-holding check (J-CRO-8).
...
- Liquidity risk and liquidity stress tests (S4, COLL 6.12.11; S5 lists liquidity risk). No J-CRO line.
...
- It shows that VaR, stress and scenario results, limits with breach handling, exposure breakdown by classification, and drill-down to holdings are standard content in both regulator text and vendor documentation. J-CRO-2, 3, 4, 6 and 8 rest on that.
- It shows that no fetched source describes the private-sleeve desmoothing comparison (J-CRO-5) or the trend charts (J-CRO-7). Those lines rest on the owner's ratification (DP-RB2-3) and on the platform's own thesis, not on an outward claim. Expected shortfall, by contrast, is named by a regulator (S3-c) and a standard-setter (S1-d) as a measure funds may or should use, so J-CRO-2's ES element does have outward support; what no source describes is ES as a headline display item (F-1).
...
An independent different-engine citation lane (Opus 5) that saw only the sources re-fetched all seven on 2026-09-18 (S3 through the WebFetch fallback after sec.gov returned 403 to `curl`), extracted the text into the session scratchpad (`cite/s1.txt` to `s7.txt`), and compared every quoted passage character by character after whitespace normalisation: 33 of 33 passages VERBATIM, none altered, none not found, none unreachable; the section 1 failure list re-tested and still true (iosco.org 403, simcorp.com 404). It re-judged all eight J-CRO mapping rows from the sources and the source rule (7 sources, 4 regulator or standard-setter, 3 vendor; both thresholds cleared). It checked 67 claims and raised 13 findings: 1 HIGH (F-1, expected shortfall IS named by S3 and S1), 5 MED (F-2 the Bloomberg date, F-3 an S1-b page off by one, F-4 currency named as a risk-model factor in S6, F-5 a quote label carrying a fact true of the source but not of the quote, F-6 three mapping rows asserting support for lines whose distinctive half no quote describes), 7 LOW (F-0 the attestation, F-7 ASCII folding undisclosed, F-8 S7-a cited for attribution, F-9 "model validation" not in the cited passages, F-10 a bullet that is an enumerated list, F-11 two S7 headings, F-12 S3 page 202 under-cited). All 13 were folded into this file on 2026-09-18 by re-reading the extracted texts (the new quotes S1-d, S3-c, S3-d, S5-e, S6-f and S7-f were copied from them and their PDF pages re-derived from the extraction's page markers); none was refuted. The findings are labelled by id where they changed a sentence.
